...
// Load ABIs
const packageManagerAbi = require(path.join(__dirname, '..', 'src', 'abi', 'PackageManager.json')).abi;
...
async function getReservesAt(pair, blockTag) {
  // Avoid historical state calls on BSC (non-archive nodes). Use latest only.
...
async function main() {
  ...
  if (txArg && txArg.startsWith('0x') && txArg.length === 66) {
    const receipt = await provider.getTransactionReceipt(txArg);
    if (!receipt) {
      console.log('Transaction receipt not found for hash:', txArg);
      return;
    }
    // Determine which PackageManager emitted Purchased in this tx
    const iface = new ethers.Interface(packageManagerAbi);
    for (const log of receipt.logs) {
      try {
        const parsed = iface.parseLog(log);
        if (parsed && parsed.name === 'Purchased') {
          chosen = { transactionHash: txArg, blockNumber: receipt.blockNumber };
          chosenPmAddr = log.address;
          break;
        }
      } catch (_) {}
    }
    if (!chosen) {
      console.log('No Purchased event found in the provided transaction.');
      return;
    }
    const pmAddrLower = chosenPmAddr.toLowerCase();
    blocksAddr = PM_BLOCKS_MAP[pmAddrLower];
  } else {
  ...
  // Decode reserves AFTER from Sync log in this transaction
  const receipt = await provider.getTransactionReceipt(txHash);
  const syncTopic = ethers.id('Sync(uint112,uint112)');
  const pairSyncLogsInTx = receipt.logs.filter(l => l.address.toLowerCase() === pairAddress.toLowerCase() && l.topics[0] === syncTopic);
  if (pairSyncLogsInTx.length === 0) {
    console.log('No Sync event found for pair in this transaction.');
    return;
  }
  const afterSyncLog = pairSyncLogsInTx[pairSyncLogsInTx.length - 1];
  const afterDecoded = ethers.AbiCoder.defaultAbiCoder().decode(['uint112','uint112'], afterSyncLog.data);

  // Identify token ordering
  const latestInfo = await getReservesAt(pair);
  const usdtIsToken0 = latestInfo.token0.toLowerCase() === USDT.toLowerCase();

  const reserve0_after = afterDecoded[0];
  const reserve1_after = afterDecoded[1];
  const reserveUSDT_after = usdtIsToken0 ? reserve0_after : reserve1_after;
  const reserveBLOCKS_after = usdtIsToken0 ? reserve1_after : reserve0_after;

  // Estimate BEFORE reserves using Mint(amount0, amount1) emitted in the same tx
  const mintTopic = ethers.id('Mint(address,uint256,uint256)');
  const pairMintLogsInTx = receipt.logs.filter(l => l.address.toLowerCase() === pairAddress.toLowerCase() && l.topics[0] === mintTopic);
  let reserveUSDT_before, reserveBLOCKS_before;
  if (pairMintLogsInTx.length > 0) {
    const lastMint = pairMintLogsInTx[pairMintLogsInTx.length - 1];
    // sender is indexed; data contains only amount0, amount1
    const decodedMint = ethers.AbiCoder.defaultAbiCoder().decode(['uint256','uint256'], lastMint.data);
    const amount0 = decodedMint[0];
    const amount1 = decodedMint[1];
    const usdtMint = usdtIsToken0 ? amount0 : amount1;
    const blocksMint = usdtIsToken0 ? amount1 : amount0;
    reserveUSDT_before = reserveUSDT_after - usdtMint;
    reserveBLOCKS_before = reserveBLOCKS_after - blocksMint;
  }
  ...
  if (liqEvent) {
    ...
    if (reserveUSDT_before !== undefined) {
      deltaUSDT = reserveUSDT_after - reserveUSDT_before;
      deltaBLOCKS = reserveBLOCKS_after - reserveBLOCKS_before;
    } else {
      deltaUSDT = actualUsdt;
      deltaBLOCKS = actualShare;
    }
    console.log('Delta            (USDT, BLOCKS):', fmt(deltaUSDT, 18), fmt(deltaBLOCKS, 18));
    const usdtMatch = (BigInt(deltaUSDT) === BigInt(actualUsdt));
    const shareMatch = (BigInt(deltaBLOCKS) === BigInt(actualShare));
