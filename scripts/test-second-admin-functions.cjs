...
async function main() {
  console.log("🧪 Testing Admin Functions for Second New Admin Wallet...");
  
  // Contract addresses
  const PACKAGE_MANAGER_ADDRESS = "0x1e44B103349598aebe2D1F33E4c42B92D0d713B3";
  const SECOND_ADMIN_WALLET = "0xfF81cBA6Da71c50cC3123b277e612C95895ABC67";
  
  console.log("🎯 Second Admin Address:", SECOND_ADMIN_WALLET);
  ...
  const hasAdminRole = await packageManager.hasRole(DEFAULT_ADMIN_ROLE, SECOND_ADMIN_WALLET);
  console.log("🔑 Second Wallet Has Admin Role:", hasAdminRole);
  
  if (!hasAdminRole) {
    console.error("❌ Second wallet does not have admin role. Cannot test functions.");
    return;
  }
  ...
  try {
    ...
    console.log("✅ Test 3 - Global Target Price:", globalTargetPrice.toString(), "wei");
  } catch (error) {
  ...
  try {
    ...
    if (allPackageIds.length > 0) {
      ...
      console.log("   Entry USDT:", firstPackage.entryUSDT.toString());
      console.log("   Exchange Rate:", firstPackage.exchangeRate.toString());
  ...
  console.log("\n🎉 Admin Function Tests Completed for Second Admin Wallet!");
  console.log("📋 The second admin wallet can now perform the following functions:");
