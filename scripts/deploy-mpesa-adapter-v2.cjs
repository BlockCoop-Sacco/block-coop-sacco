...
async function main() {
  console.log('🚀 Deploying MpesaTreasuryAdapterV2');
  ...
  const Adapter = await ethers.getContractFactory('MpesaTreasuryAdapterV2');
  const adapter = await Adapter.deploy(usdt, pm, treasury, backend);
  await adapter.waitForDeployment();
  const adapterAddress = await adapter.getAddress();
  console.log('✅ Deployed MpesaTreasuryAdapterV2 at:', adapterAddress);

  d.mpesaAdapterV2 = adapterAddress;
  d.timestamp = Date.now();
  fs.writeFileSync(deploymentsPath, JSON.stringify(d, null, 2));
  console.log('📝 Updated', deploymentsPath, 'with mpesaAdapterV2');
}
